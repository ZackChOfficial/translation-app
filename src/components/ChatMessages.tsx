...
function ChatMessages({initialMessages, session, chatId}: Props) {
  ...
  return (
    <div className='p-5'>
        {!loading && messages?.length == 0 && (
            <div className='flex flex-col justify-center text-center items-center p-20 rounded-xl space-y-2 bg-indigo-400/30 text-white font-extralight'>
                <MessageCircleIcon className='h-10 w-10'/>
                <h2>
                    <span className='font-bold'>Invite a friend</span> &{" "}
                    <span className='font-bold'>Send your message in ANY language </span>
        ...
        {messages?.map(message => {
            ...
            return <div key={message.id} className='flex my-2 items-end'>
